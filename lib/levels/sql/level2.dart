...
class _SqlLevel2State extends State<SqlLevel2> {
  ...
  Future<void> saveScoreToPrefs(int score) async {
    ...
    await prefs.setInt('sql_level2_score', score);

    if (score > 0) {
      await prefs.setBool('sql_level2_completed', true);
    }
  }

  Future<void> loadScoreFromPrefs() async {
    final prefs = await SharedPreferences.getInstance();
    final savedScore = prefs.getInt('sql_level2_score');
    final completed = prefs.getBool('sql_level2_completed') ?? false;
    setState(() {
  ...
  void checkAnswer() async {
    ...
    if (answer == 'SELECT * FROM students ;') {
      countdownTimer?.cancel();
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: const Text("Nice! You wrote a valid SQL query."),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(context);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("🗄️ SQL - Level 2"),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            isTagalog
                ? 'Si Zeke ay gustong makita ang lahat ng estudyante mula sa database. Tulungan siyang buuin ang tamang SELECT query!'
                : 'Zeke wants to view all students from the database. Help him build the correct SELECT query!',
            textAlign: TextAlign.justify,
            style: const TextStyle(fontSize: 16),
          ),
          const SizedBox(height: 20),
          const Text('🧩 Arrange the blocks to form: SELECT * FROM students;',
