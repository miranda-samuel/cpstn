...
class _PythonLevel6State extends State<PythonLevel6> {
  ...
  void resetBlocks() {
    // 🔹 Puzzle blocks for Level 6 (if-else statement)
    allBlocks = [
      'if',
      'x',
      '%',
  ...
  Future<void> saveScoreToPrefs(int score) async {
    ...
    await prefs.setInt('python_level6_score', score);

    if (score > 0) {
      await prefs.setBool('python_level6_completed', true);
    }
  }

  Future<void> loadScoreFromPrefs() async {
    final prefs = await SharedPreferences.getInstance();
    final savedScore = prefs.getInt('python_level6_score');
    final completed = prefs.getBool('python_level6_completed') ?? false;
    setState(() {
      if (savedScore != null) score = savedScore;
      level6Completed = completed;
    });
  }
  ...
  void checkAnswer() async {
    ...
    // 🔹 Correct answer for Level 6
    if (answer == 'if x % 2 == 0: print ("Even") else: print ("Odd")') {
      countdownTimer?.cancel();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("🐍 Python - Level 6"),
  ...
  Widget buildStartScreen() {
    return Center(
      child: Column(
        ...
        children: [
          ElevatedButton.icon(
            onPressed: level6Completed ? null : startGame,
            icon: const Icon(Icons.play_arrow),
            label: Text(level6Completed ? "Completed" : "Start Game"),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            isTagalog
                ? 'Gumawa ng if-else statement na magpi-print ng "Even" kung x % 2 == 0 at "Odd" kung hindi.'
                : 'Write an if-else statement that prints "Even" if x % 2 == 0 else prints "Odd".',
            textAlign: TextAlign.justify,
            style: const TextStyle(fontSize: 16, color: Colors.white),
          ),
          const SizedBox(height: 20),
          const Text(
              '🧩 Arrange the puzzle blocks to form: if x % 2 == 0: print("Even") else: print("Odd")',
