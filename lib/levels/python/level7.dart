...
class PythonLevel7 extends StatefulWidget {
  const PythonLevel7({super.key});

  @override
  State<PythonLevel7> createState() => _PythonLevel7State();
}

class _PythonLevel7State extends State<PythonLevel7> {
  ...
  Future<void> saveScoreToPrefs(int score) async {
    ...
    await prefs.setInt('python_level7_score', score);
    if (score > 0) {
      await prefs.setBool('python_level7_completed', true);
    }
  }

  Future<void> loadScoreFromPrefs() async {
    final prefs = await SharedPreferences.getInstance();
    final savedScore = prefs.getInt('python_level7_score');
    final completed = prefs.getBool('python_level7_completed') ?? false;
    setState(() {
      if (savedScore != null) score = savedScore;
      level7Completed = completed;
    });
  }
  ...
  void checkAnswer() async {
    ...
    // 🔹 Correct answer for Level 7
    if (answer == 'def greet(name): return "Hello + name print ( greet("James") )') {
      countdownTimer?.cancel();
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: const Text(
            "Output:\nHello James", // example output of greet("James")
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("🐍 Python - Level 7"),
  ...
  Widget buildStartScreen() {
    return Center(
      child: Column(
        ...
        children: [
          ElevatedButton.icon(
            onPressed: level7Completed ? null : startGame,
            icon: const Icon(Icons.play_arrow),
            label: Text(level7Completed ? "Completed" : "Start Game"),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            isTagalog
                ? 'Gumawa ng function greet(name) na magbabalik ng "Hello" + name at tawagin ito gamit ang print(greet("James")).'
                : 'Write a function greet(name) that returns "Hello" + name and call it with print(greet("James")).',
          ...
          const Text(
              '🧩 Arrange the puzzle blocks to form:\n'
                  'def greet(name): return "Hello " + name print(greet("James"))',
