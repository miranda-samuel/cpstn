...
class _PhpLevel1State extends State<PhpLevel1> {
  ...
  Future<void> saveScoreToPrefs(int score) async {
    ...
    await prefs.setInt('php_level1_score', score);

    if (score > 0) {
      await prefs.setBool('php_level1_completed', true);
    }
  }

  Future<void> loadScoreFromPrefs() async {
    final prefs = await SharedPreferences.getInstance();
    final savedScore = prefs.getInt('php_level1_score');
    final completed = prefs.getBool('php_level1_completed') ?? false;
    setState(() {
  ...
  void checkAnswer() async {
    ...
    if (answer == '<?php echo "Hello World"; ?>') {
      countdownTimer?.cancel();
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: const Text("Nice! You printed Hello World in PHP."),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(context);
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            isTagalog
                ? 'Si Zeke ay unang natututo ng PHP! Gusto niyang ipakita ang kanyang unang output gamit ang echo "Hello World". Pwede mo ba siyang tulungan buuin ang tamang code?'
                : 'Zeke is learning PHP for the first time! He wants to display his first output using echo "Hello World". Can you help him build the correct code?',
            textAlign: TextAlign.justify,
            style: const TextStyle(fontSize: 16),
          ),
          const SizedBox(height: 20),
          const Text('🧩 Arrange the puzzle blocks to form: <?php echo "Hello World"; ?>',
